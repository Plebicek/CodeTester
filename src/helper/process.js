...
export const unzipFolder = async function (testFolder) {
    try {
        await decompress(path.join(process.cwd(),`/java/tests/${testFolder}/src/main.zip`), path.join(process.cwd(),`/java/tests/${testFolder}/src/main`))
    } catch(err) {
 ...
 */
const runProcess = async function({filename, testFolder}) {
    console.log(filename, testFolder)
    try {
        await moveZipToTestFolder({filename,testFolder})
