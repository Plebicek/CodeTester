  ...
  const javaUploadPath = JAVA_UPLOAD+`test_${testId}.zip`
    ...
    (err) => {
      ...
      try {
        ...
        console.log(
          `While removing the file ${testId} error has occured ` + err); //ToDo set to delayed queue
        return done()
      } 
    }
    )
    rename(javaTestPath,
      JAVA_TEST+`${testId}.zip`,
      (err) => {
        if (err) {
    ...
    await new Promise(resolve => setTimeout(async ()=>await updateTest(testId, job.data.task_id), 5000));
  }); 
